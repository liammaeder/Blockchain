...
class Blockchain{
    ...
    createGenesisBlock(){  
        return new Block(Date.parse('20/05/2023'), [], '0', '0');
    }
    ...
    isChainValid(){
        for(let i = 1; i < this.chain.length; i++){
            ...
            if(!currentBlock.hasValidTransactions()){
                throw new Error('Invalid transactions exist.');
            }

            // check if current block's data has been changed
            if(currentBlock.hash !== currentBlock.calculateHash()){
                throw new Error('Current block data has changed.');
            }
            
            // Does current block point to previous block
            if(currentBlock.previousHash !== previousBlock.calculateHash()){
                throw new Error('Current block does not point to previous block.');
            }
        }
        return true;
    }
